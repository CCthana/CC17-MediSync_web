...
// const getDocumentDefinition = (data) => ({
//   pageSize: "A4",
...
//     { text: `ชื่อ ${data.doctor.firstName} ${data.doctor.lastName}`, style: "subheader" },
//     {
...
// export const sendPdfMedicalCertificate = (data) => {
...
const getDocumentDefinition = (data, imageBase64) => ({
  ...
  content: [
    ...
    { text: `เลขที่ ${data.id}-${data.vn}`, alignment: "right" },
    { text: `ชื่อ ${data.doctor.firstName} ${data.doctor.lastName}`, style: "subheader" },
    { text: `ได้ทำการตรวจ คุณ${data.user.firstName} ${data.user.lastName} เลขที่ HN: ${data.hn}`, margin: [0, 0, 0, 0] },
    ...
    { text: "พิมพ์ผลวันที่ 12/07/2567", absolutePosition: { x: 375, y: 760 }, alignment: "right" },
  ],
  defaultStyle: {
    font: "THSarabunNew",
  },
